...
local function loadMainMenu()
    ...
    local Window = Rayfield:CreateWindow({
        ...
        LoadingSubtitle = "by namesick | Ключ активен",
    ...
    local function openDiscord()
        ...
        if shell and shell.open then
            shell.open(DISCORD_URL)
            opened = true
        elseif os and os.execute then
            os.execute('start "' .. DISCORD_URL .. '"')
            opened = true
        ...
        print("🌐 Открыт Discord: " .. DISCORD_URL)
        return true
    end
    ...
    local CopyButton = TabInf:CreateButton({
        ...
        Callback = function()
            setclipboard(DISCORD_URL)
            print("📋 Ссылка скопирована: " .. DISCORD_URL)
        end,
    })
    ...
    local function formatTime(seconds)
        ...
        if days > 0 then
            return string.format("%dд %02dч %02dм %02dс", days, hours, minutes, secs)
        elseif hours > 0 then
            return string.format("%02dч %02dм %02dс", hours, minutes, secs)
        elseif minutes > 0 then
            return string.format("%02dм %02dс", minutes, secs)
        else
            return string.format("%02dс", secs)
        end
    end
