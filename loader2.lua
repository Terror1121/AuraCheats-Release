...
local function checkDebug()
    ...
    -- Если debug недоступен - подозрительно, но пропускаем
    if not success then
        print("⚠️ Debug недоступен, пропускаем проверку")
        return true
    end
...
-- 2. ПРОВЕРКА ИНЖЕКТОРА (РАСШИРЕННЫЙ СПИСОК)
...
if not checkInjector() then 
    print("❌ Неподдерживаемый инжектор, остановка")
    while true do task.wait(999999) end 
end
...
local function checkSystemTime()
    ...
    local minTime = os.time({year=2024, month=1, day=1})
    
    if currentTime < minTime then
        print("🚫 Обман времени! Текущее: " .. os.date("%Y-%m-%d", currentTime))
        return false
    end
...
local function checkIntegrity()
    local protected = {"checkInjector","checkSystemTime","activateKeyThroughBot","loadMainScript","checkVersion"}
...
local function checkVersion()
    ...
    local p1 = split(CURRENT_VERSION)
    local p2 = split(latestVersion)
...
-- 7. СИСТЕМА КЛЮЧЕЙ
local keyData = {isValid=false, activationDate=nil, expirationDate=nil, key=nil}
...
-- 11. GUI ВВОДА КЛЮЧА (ОКНО АКТИВАЦИИ)
    ...
    activateBtn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            ...
            if valid then
                ...
                saveKeyData({key=key, activationDate=activationTime, expirationDate=expTime})
                task.wait(1.5)
                screenGui:Destroy()
                loadMainScript()
            else
